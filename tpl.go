...
var fsmTemplate = `// Code generated by go-fsm (http://github.com/MrEhbr/go-fsm); DO NOT EDIT.
...
var (
	...
	// Err{{.Struct.Name}}FsmSkip indicates that further processing is not needed
...
func (m *{{.Struct.Name}}StateMachine) ChangeState(ctx context.Context, event string, obj *{{.Struct.Name}}) error {
	...
	if len(trans.BeforeActions) > 0 && m.actionHandler != nil {
		for _, action := range trans.BeforeActions {
			if err := m.actionHandler(ctx, action, trans, obj); err != nil {
				...
				return fmt.Errorf("%w. action [%s] returned error: %w", Err{{.Struct.Name}}FsmBeforeAction, action, err)
			}
		}
	}
	...
	if len(trans.Actions) > 0 && m.actionHandler != nil {
		...
		for _, action := range trans.Actions {
			if err := m.actionHandler(ctx, action, trans, obj); err != nil {
				errs = append(errs, fmt.Errorf("%w. action [%s] returned error: %w", Err{{.Struct.Name}}FsmAction, action, err))
			}
		}
